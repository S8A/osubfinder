...
class OpenSubtitlesHasher {
    ...
    static String computeHash(InputStream stream, long length) 
        ...
        byte[] chunkBytes = new byte[(int) Math.min(
                2 * HASH_CHUNK_SIZE, length)];
        ...
        /* seek to position of the tail chunk, or not at all if 
         * length is smaller than two chunks */
        while (position < tailChunkPosition 
                && (position += in.skip(tailChunkPosition - position)) >= 0);
        
        /* second chunk, or the rest of the data if length is 
         * smaller than two chunks */
        in.readFully(chunkBytes, chunkSizeForFile, 
                     chunkBytes.length - chunkSizeForFile);
        
        long head = computeHashForChunk(ByteBuffer.wrap(
                chunkBytes, 0, chunkSizeForFile));
        long tail = computeHashForChunk(ByteBuffer.wrap(
                chunkBytes, chunkBytes.length - chunkSizeForFile, 
                chunkSizeForFile));
